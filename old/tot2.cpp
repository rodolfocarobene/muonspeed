...
double myFun2(double *x, double *par){		
	double tau0 = par[0];		//tau
	double vth0 = par[1];		//Vth
	double off0 = par[2];		//offset
	double tau1 = par[3];		//tau
	double off1 = par[4];		//offset
	double D = x[0];
	double mean = 28;
	if(D >= mean) return tau1*log(D/(D-vth0)) + off1;
	else return tau0*log(D/(D-vth0))+off0;
}
...
int main(int argc, char *argv[]){
	...
	for (int i = 0; i < nbin2; i++){
		...
		if (x2 > mins2_lan && x2 < maxs2_lan){
			...
			x2 = meanx2[i];
			int n = gr_0 -> GetN();
			if (count > 0 && x2 > 0) gr_0 -> SetPoint(n, x2, y);
			meany2.push_back(y);
			stdy2.push_back(std);
			double xerr = stdx2[i];
			if(xerr == 0) xerr = 1;
			if(std == 0) std = 1;
			if (count > 0 && x2 > 0) gr_0 -> SetPointError(n, xerr, std);
	...
	TF1 * fun_adc0 = new TF1("fun_adc0",myFun2, 0,300,5);
	...
	fun_adc0 -> SetParName(3, "Tau1");
	fun_adc0 -> SetParName(4, "Off1");

	fun_adc1 -> SetParName(0, "Tau1");
	fun_adc1 -> SetParName(1, "Vth1");
	fun_adc1 -> SetParName(2, "Off1");

	fun_adc0 -> SetParameter(0, 16.72);
	fun_adc0 -> SetParameter(1, -31);
	fun_adc0 -> SetParameter(2, 280);
	fun_adc0 -> SetParameter(3, 44);
	fun_adc0 -> SetParameter(4, 270);


	fun_adc1 -> SetParameter(0, 30);
